...
class TensorPlayer:
    ...
    def action(self,env,can_stop=True):
        ...
        my_action = int(np.random.choice(range(self.move_n), p = improved_policy))
    ...
    #         = less.
    #     """
    #     tau = np.power(self.playconfig.tau_decay_rate, turn + 1)
    #     if tau < 0.1:
    #         tau = 0
    #     if tau == 0:
    #         action = np.argmax(policy)
    #         ret = np.zeros(self.move_n)
    #         ret[action] = 1.0
    #         return ret
    #     else:
