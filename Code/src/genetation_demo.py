...
def generate_demo(S, R_limit, p_entry, N, random_seed, value_set=[-1, 0, 1], len_chain=7):
    ...
    for i in range(len(demo)):
        ...
        if R_limit == len_chain:
            ...
            #只能生成一组数据
            tensor_chain = rank_one_tensor[::-1]
            action = (factors_U[:,-1], factors_V[:,-1], factors_W[:,-1])
            index = torch.tensor([6,5,4,3,2,1])
            value = 1.0
            state_plane_tensor.append((tensor_chain, index, action, value))
        else:
            ...
            for i in range(R_limit - len_chain - 1): 
                ...
                value = 1.0 if (i == R_limit - len_chain - 1) else 0.5
    ...
    return state_plane_tensor, state_plane_index, action, value
