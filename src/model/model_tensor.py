...
class AlphaTensorNet(nn.Module):
    ...
    def train(self, x, s, g_action, g_value, c=512):
        """
    ...
    def policyhead_training(self, e, g):
        """
        ...
        print(f"onehot_shifted_g.shape : {onehot_shifted_g.shape}")
        # Training by teacher-forcing
        o, z = self.predict_action_logits(g, e)
        return o,z[0,0] 
    ...
    def policyhead_inference(self, e, N_steps, N_logits, N_samples=32):
        ...
        """
        device = e.device
        a = torch.zeros(N_samples, N_steps, N_logits, dtype=torch.long, device=device)
        p = torch.ones(N_samples, device=device)
        
        for s in range(N_samples):
            for i in range(N_steps):
                logits, z = self.predict_action_logits(a[s, :i], e)
                probs = torch.softmax(logits, dim=-1)
                a[s, i] = torch.multinomial(probs, 1)
                p[s] *= probs[a[s, i]]
        
        _, z = self.predict_action_logits(a[0], e)
    ...
    def attention(self, x, y, causal_mask=False, Num_Head=16, d=32, w=4):
        ...
        q = q_linear(x_norm).view(-1, Num_Head, N_x, d)
        k = k_linear(y_norm).view(-1, Num_Head, N_y, d)
        v = v_linear(y_norm).view(-1, Num_Head, N_y, d)
    ...
    def shifted(g):
        # size of g: (batch_size, seq_len)
        shifted_g = torch.roll(g, shifts=1, dims=1)  # 右移一位
        shifted_g[:, 0] = 0  # 序列开头填充起始符（假设0是有效类别索引）
        return shifted_g
